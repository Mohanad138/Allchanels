...
def check_is_ad(text, api_key):
    prompt = f"""حدد فقط إذا كان النص التالي إعلاناً مدفوعاً أو محتوى ترويجياً تجارياً صريحاً.
اعتبره إعلاناً فقط لو كان يروّج مباشرة لمنتج، تطبيق، خدمة، قناة/حساب آخر، أو رعاية مدفوعة، أو يحتوي كود خصم/دعوة للشراء أو الاشتراك بشيء تجاري.
أي خبر عادي — حتى لو ذكر جوائز، مسابقات، إعلانات رسمية من جهة حكومية، أو أحداث — **ليس إعلاناً**، اعتبره is_ad=false.
عند الشك، اجعل is_ad=false (انشر). لا تعدل النص إطلاقاً.
